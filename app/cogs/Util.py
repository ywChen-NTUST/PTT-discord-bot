...
class Util(commands.Cog):
	...
	@nextcord.slash_command(guild_ids=[GUILD_ID], description=DESCRIPTION["Util"]["ping"]["describe"])
	async def ping(self, interaction: nextcord.Interaction) -> None:
		latency = round(self.bot.latency * 1000, 1)
		await interaction.response.send_message(f"pong!\nlatency: {latency} ms")
	
	@nextcord.slash_command(guild_ids=[GUILD_ID], description=DESCRIPTION["Util"]["ping_ptt"]["describe"])
	...
	@nextcord.slash_command(guild_ids=[GUILD_ID], description=DESCRIPTION["Util"]["help"]["describe"])
